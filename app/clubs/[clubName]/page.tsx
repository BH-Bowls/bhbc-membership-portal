              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-2">
                    {contactRoles.map((r) => {
                      const selected = newContact.role.split(',').map(s => s.trim()).includes(r);
                      return (
                        <label key={r} className="flex items-center gap-1.5 cursor-pointer">
                          <input
                            ...
                            checked={selected}
                            onChange={() => {
                              const parts = newContact.role.split(',').map(s => s.trim()).filter(Boolean);
                              const next = selected
                                ? parts.filter(p => p !== r)
                                : [...parts, r];
                              setNewContact({ ...newContact, role: next.join(',') });
                            }}
                            className="w-4 h-4"
                          />
                    ...
                    <div>
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="md:col-span-2">
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-2">
                            {contactRoles.map((r) => {
                              const selected = (editedContact.role || '').split(',').map(s => s.trim()).includes(r);
                              return (
                                <label key={r} className="flex items-center gap-1.5 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={selected}
                                    onChange={() => {
                                      const parts = (editedContact.role || '').split(',').map(s => s.trim()).filter(Boolean);
                                      const next = selected
                                        ? parts.filter(p => p !== r)
                                        : [...parts, r];
                                      setEditedContact({ ...editedContact, role: next.join(',') });
                                    }}
                                    className="w-4 h-4"
                                  />
                                  <span className="text-sm text-gray-900">{r}</span>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {contact.role && contact.role.split(',').map(r => r.trim()).filter(Boolean).map(r => (
                            <span key={r} className="px-2 py-0.5 text-xs bg-blue-100 text-blue-700 rounded">{r}</span>
                          ))}
                        </div>

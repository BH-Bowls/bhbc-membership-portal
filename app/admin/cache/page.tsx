...
function CacheSection({ title, subtitle, countLabel, stats }: { title: string; subtitle: string; countLabel: string; stats: CacheStats }) {
  let hitRate = '—';
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
      ...
      <dl className="grid grid-cols-2 gap-y-1.5 text-sm mb-3">
        <dt className="text-gray-700">{countLabel}</dt>
        <dd className="text-gray-900 font-medium">{stats.cached ? stats.count : '—'}</dd>
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="mb-4">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-900">Cache View</h1>
            <button onClick={load} disabled={loading} className={getButtonClasses('secondary', 'sm')}>
              {loading ? 'Refreshing…' : 'Refresh'}
            </button>
          </div>
          <p className="text-sm text-gray-700 mt-1">
            Sheets are cached in memory to cut Google Sheets read-quota usage. Any write to a cached
            sheet drops that cache immediately.
          </p>
        </div>
        ...
        {usersCache && (
          <CacheSection
            ...
            subtitle="Used by getAllUsers — auth, profile, competitions, leagues, admin, and most member lookups. TTL 24h."
